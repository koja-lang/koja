...
- `koja test` flags. `--reporter <name>` selects `dots` (the default), `trace`, or `json`, and `--trace` stays as an alias for `--reporter trace`. The `json` reporter writes one event per line to stderr, or to a file with `--out <path>`, for CI and editors, with each test's time as integer `microseconds`. The human reporters print the time in the unit that fits, `452µs`, `3ms`, or `1.2s`. `--timeout <ms>` sets the deadline for each test. `--backend {interpreter,llvm}` picks the backend the way `koja run` does.
...
- Calendar and zone types on top of `Timestamp`. The `DateTime` name returns as the zoned type, see the Removed entry below.
  - `Date`, `Time`, and `LocalDateTime`, civil values with no zone. `Date.new` and `Time.new` check every field, so a value is always valid. `Date` has `day_of_week`, `day_of_year`, `plus_days`, `plus_months` (clamping to the month's end), `plus_years`, and the epoch-day conversions. `Time` has microsecond precision and converts to and from a `Duration` since midnight. `Weekday` runs `Monday` to `Sunday`.
  - `TimeZone`, an enum with `UTC`, `Fixed(TimeZone.Offset)`, and `Named(TimeZone.Rules)`. `TimeZone.Offset` is a distance from UTC within -18:00..+18:00, with the constant `TimeZone.Offset.UTC`.
  - `TimeZone.Rules`, the data behind a named zone. It holds the IANA identifier, the offset before the first transition, a sorted list of `TimeZone.Transition` values, and a `TimeZone.Rule` for the years after the last one. `TimeZone.Rule` holds the standard offset and an optional `TimeZone.Rule.Daylight` with the daylight offset and two `TimeZone.Rule.Point` values, the POSIX `Mm.w.d/time` form. The stdlib evaluates the rules and ships no data. The `koja-lang/tz` package embeds the IANA database and builds a `Named` zone for an identifier such as `America/Chicago`.
  - `TimeZone.Resolution`, returned wherever a civil time is placed in a zone: `Unique`, `Gap`, or `Ambiguous`, with `compatible()`, `earlier()`, `later()`, and `unique()`. `LocalDateTime.in_zone` resolves against the offsets in force around the clock time, so a `Named` zone reports its skipped hour as `Gap` and its repeated hour as `Ambiguous`. `UTC` and `Fixed` always resolve to `Unique`.
  - `DateTime`, a `Timestamp` viewed in a `TimeZone`. `date`, `time`, `local`, and `offset` compute from the two fields. `plus` and `minus` take a `Duration` and are exact. `plus_days` and `plus_months` keep the clock time and return a `TimeZone.Resolution`. Two values are equal when they name the same instant in the same zone.
  - A format protocol per type, `Date.Format`, `Time.Format`, `LocalDateTime.Format`, and `DateTime.Format`, each with a `format_*` and a `parse_*` method. Every type has `to_string(format)` and `parse(text, format)` that forward to it, with `ISO8601.Extended` as the default format, so `Date.parse("2026-09-16")` and `date.to_string()` need no argument.
  - `ISO8601`, the stdlib format, with `Extended` (`2026-09-16T17:39:00Z`) and `Basic` (`20260916T173900Z`) variants. A `DateTime` in `Extended` form is RFC 3339. A parse reports an out-of-range field with that field's error and any other shape as `Malformed`.
...
- `koja deps outdated` lists the git dependencies whose remote has something newer than the pin. A `tag` pin is compared against the remote's `X.Y.Z` tags, with an optional `v`, and prints `tz 2026.3.0 -> 2026.4.0`. A `branch`, `rev`, or default-branch pin is compared against the remote head. The command exits 1 when anything is outdated, so CI can run it, and never writes `koja.lock`.
...
- **Breaking change.** `Duration` is a `value` and a `Duration.Unit`, from `Nanoseconds` to `Hours`, built with `Duration.new(30, Duration.Unit.Seconds)`. It stores what the caller wrote, so no constructor can overflow. Two durations are equal when they cover the same span, and `plus` and `minus` return the finer unit. The accessors spell out their unit, `to_hours` through `to_nanoseconds`, and truncate toward a coarser unit. `Duration.from_secs(n)` becomes `Duration.new(n, Duration.Unit.Seconds)`, and `millis()` becomes `to_milliseconds()`.
...
- `@test` is deprecated and will be removed in 0.20. Every annotation reports a warning that points at the `test "description"` block. The function still runs under `koja test` until then.
...
- A generic parameter can have a default, `fn parse<F: Reader>(text: String, format: F = Plain.Decimal)`. Calling it without the argument failed with "cannot infer type parameter `F`" because the shorter arity kept a type parameter no argument could bind. The type now comes from the default.
...
- **Breaking change.** `DateTime` as the epoch time type is removed. That role moves to `Timestamp`, and the name now belongs to the zoned calendar type in the Added section. `DateTime.now().timestamp_millis()` becomes `Timestamp.now().since_epoch.to_milliseconds()`, and two wall-clock reads subtracted to time something become one `Instant`. Check every `DateTime` annotation by hand. The new `DateTime.now` takes a `TimeZone`, so a zero-argument call no longer compiles, but an annotation that never calls a method compiles against the new type and means a different thing.
...
- `==` and Debug output on maps and sets inspect their contents instead of using fieldless derived results.
...
- A conditional top-level `return` in a script no longer crashes `koja run --backend=llvm`.
