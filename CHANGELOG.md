...
- Calendar and zone types on top of `Timestamp`. The `DateTime` name returns as the zoned type, see the Removed entry below.
  ...
  - `TimeZone`, an enum with `UTC` and `Fixed(TimeZone.Offset)`. `TimeZone.Offset` is a distance from UTC within -18:00..+18:00, with the constant `TimeZone.Offset.UTC`. Zones with daylight saving rules arrive in a later release as a new variant from a separate package.
  - `TimeZone.Resolution`, returned wherever a civil time is placed in a zone: `Unique`, `Gap`, or `Ambiguous`, with `compatible()`, `earlier()`, `later()`, and `unique()`. The stdlib zones always resolve to `Unique`, and the enum is there so a `match` written today still compiles correctly when a zone with transitions exists.
