...
- Find references, rename, and document highlight in the editor. Find references lists every use of a function, type, constant, local, or type parameter across the project. Rename rewrites all of them in one edit and refuses, with a reason, when the program has errors, when the symbol is declared in the stdlib, or when it is `self` or a method that implements a protocol. Document highlight marks the uses in the open file and tells reads from writes.
- `crates/koja-lsp/FEATURES.md` lists the editor features the language server supports, the ones it declines, and the rename limits.
