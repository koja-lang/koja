...
- The time types. `DateTime` is now `Timestamp` and counts microseconds since the Unix epoch. `Timestamp.new(value, unit)` accepts an epoch value in any unit, and `since_epoch()` returns the `Duration` from the epoch, so `t.since_epoch().to_seconds()` is Unix time. `Duration` is a `value` and a `Duration.Unit`, from `Nanoseconds` to `Hours`, built with `Duration.new(30, Duration.Unit.Seconds)`. It stores what the caller wrote, so no constructor can overflow. Two durations are equal when they cover the same span, and `plus` and `minus` return the finer unit. The accessors spell out their unit, `to_hours` through `to_nanoseconds`, and truncate toward a coarser unit. The new `Instant` reads the monotonic clock and is the type for elapsed time: `started = Instant.now()` then `started.elapsed()`. To migrate, `DateTime.now().timestamp_millis()` becomes `Timestamp.now().since_epoch().to_milliseconds()`, `Duration.from_secs(n)` and `from_millis(n)` become `Duration.new(n, Duration.Unit.Seconds)` and `Duration.new(n, Duration.Unit.Milliseconds)`, `millis()` becomes `to_milliseconds()`, and any two wall-clock reads subtracted to time something become one `Instant`. A stale `DateTime` gets a compile error that names the new type.
