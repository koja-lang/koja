...
- **Internal**: 137 stdlib intrinsic stubs across `lib/global` and `lib/net` migrated from `panic("intrinsic")` placeholder bodies to `@intrinsic` annotations on bodyless declarations. Codegen routes annotation-keyed dispatch instead of name-pattern matching for source-declared intrinsics.
- **Internal**: 303 `print(x)` call sites across stdlib, lang_suite fixtures, and examples migrated to `x.print()`. 45 `.stdout` fixtures updated to reflect the new quoted-string convention. The `print` / `print_<Primitive>` typecheck stub, `BuiltinCall::Print` IR variant, the now-empty `BuiltinCall` enum, the `ResolvedCall::Builtin` variant, and the codegen `compile_print` function are gone.
- **Internal**: `Bool.format` intrinsic now emits a length-prefixed `String` global (via `create_string_global`) instead of a raw C string literal, so `IO.puts(true.format())` and other String-shaped consumers (rather than just `printf`) work correctly.
