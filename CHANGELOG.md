...
- Calendar and zone types on top of `Timestamp`. The `DateTime` name returns as the zoned type, see the Removed entry below.
  ...
  - `TimeZone.Rules`, the data behind a named zone: the IANA identifier, the offset before the first transition, a sorted list of `TimeZone.Transition` values, and a `TimeZone.Rule` for the years after the last one. `TimeZone.Rule` holds the standard offset and an optional `TimeZone.Rule.Daylight` with the daylight offset and two `TimeZone.Rule.Point` values, the POSIX `Mm.w.d/time` form. The stdlib evaluates the rules and ships no data. The `koja-lang/tz` package embeds the IANA database and builds a `Named` zone for an identifier such as `America/Chicago`.
