...
- TLS support in the `net` package, built on the bundled BoringSSL via C FFI. `TLSSession` wraps a `SSL`/`SSL_CTX` pair with `connect` (client) / `accept` (server) handshakes and `read_binary` / `write` / `shutdown` operations. `TCPSocket` integrates it transparently: `connect_tls(host, port)` establishes a verified TLS connection (certificate chain and hostname are checked against the system CA bundle by default), `connect_tls_with` / `upgrade_tls` / `accept_tls` cover custom configs and server-side handshakes, `tls?` reports session state, and `read` / `write` / `close` dispatch through the active session automatically. `TLSConfig.client()` / `.insecure()` / `.server(cert, key)` configure verification and server credentials, and `.with_ca(ca)` swaps the system trust store for a custom CA bundle. Credentials are in-memory `Crypto.Certificate` / `Crypto.PrivateKey` values parsed from PEM text (read from a file, environment variable, or secret store); `parse` validates the PEM labels up front, so a swapped certificate/key pair is a type error and a passphrase-protected key fails with a clear message instead of a handshake error. Server credentials travel as a `TLSIdentity`, and the trust source is an explicit `TrustStore.System` / `TrustStore.Custom(cert)`. The HTTP client now routes `https://` URLs through `connect_tls`, so `HTTP.get("https://...")` works out of the box.
- Implicit numeric widening (hub-only): a sized numeric value now flows into an `Int` or `Float` slot without an explicit conversion — `Int8` / `Int16` / `Int32` / `UInt8` / `UInt16` / `UInt32` widen to `Int`, and `Float32` widens to `Float`. Widening applies at call arguments, struct fields, enum payloads, returns, annotated bindings, and assignments; it is always lossless (signed sources sign-extend, unsigned sources zero-extend), so a negative `Int32` from C FFI stays negative. Sideways widening (`Int8` to `Int16`), narrowing, `UInt64` to `Int`, binary operators, and generic inference are unaffected — those still require exact types.
- Checked numeric narrowing methods: `Int.to_int8` / `to_int16` / `to_int32` / `to_uint8` / `to_uint16` / `to_uint32` / `to_uint64` and `UInt64.to_int`, each returning `Result<T, NumericConversionError>` with `Err(NumericConversionError.OutOfRange)` when the value does not fit. `Float.to_float32` is total and rounds to the nearest representable value.
...
- **Breaking**: the `net` package reports failures with typed errors instead of strings. `TCPSocket`, `TCPListener`, and `UDPSocket` return `Result<_, Net.SocketError>` — a cause-based enum (`ConnectionRefused`, `TimedOut`, `NameNotFound`, `AddressInUse`, ..., `Unknown(errno)`) built from stable runtime error codes, so callers can match on the cause instead of parsing message text. TLS-capable operations (`connect_tls`, `upgrade_tls`, `accept_tls`, and `read`/`write` on a TLS socket) return the union `Result<_, Net.SocketError | Net.TLSError>`; `Net.TLSError` names TLS-specific causes (`CertificateInvalid`, `PrivateKeyInvalid`, `TrustStoreInvalid`, `PeerClosed`, `ProtocolFailed`, ...) and `TLSError.VerificationFailed(VerificationError)` carries why certificate verification failed (`Expired`, `HostnameMismatch`, `SelfSigned`, `UntrustedRoot`, ..., `Other(code)`). `Crypto.Certificate.parse` / `Crypto.PrivateKey.parse` return `Result<_, Crypto.PEMError>` (`NoCertificateBlock`, `CertificateIsPrivateKey`, `EncryptedKeyUnsupported`, ...). All error enums expose `message()` for display; update callers that concatenated or matched the old `String` errors. `TCPEvent.Error` now carries a `SocketError`, and `TLSSession.read` (string-decoding read) is removed — use `TCPSocket.read`, which decodes UTF-8 and reports `SocketError.InvalidUTF8`.

- **Breaking**: `Int.parse` and `Float.parse` (and the `String.to_int` / `String.to_float` wrappers) now return `Result<T, NumericConversionError>` instead of `Result<T, String>`. Failures are matchable: `NumericConversionError.InvalidFormat` for malformed text, `NumericConversionError.OutOfRange` for well-formed numbers that don't fit (an integer overflowing 64 bits — previously reported as invalid — or a float magnitude like `1e999` that would round to infinity — previously silently parsed as `inf`). Only finite floats parse; the `inf` / `nan` tokens are rejected as `InvalidFormat`. Update any caller matching `Result.Err(msg)` as a `String`.
...
- Fixed an unbounded memory leak on extremely common patterns: a heap-backed temporary passed as an argument (`f(a <> b)`), used as a method receiver in a chain (`make().foo().bar()`), or produced by a literal/constructor (`[1, 2, 3]`, `Msg{text: a <> b}`, `Shape.Named{...}`) was never released, so the allocation leaked once per evaluation. Callers now drop the heap temporaries they hand to a clone-on-entry callee, construction results are owned and moved (not cloned) into their binding, and control-flow joins (`if`/`cond`/`match`/`receive`) own their merged heap result. Long-running programs (e.g. servers) no longer bleed memory on routine work.
- Fixed a native-backend stack overflow in long-running processes: a self-recursive tail call written as the _value_ of an `if` / `match` / `receive` (notably the `receive ... after -> self.loop()` actor loop) was not tail-call-optimized, so the loop grew the stack one frame per iteration until it crashed. Tail-call optimization now sees through these control-flow merges and the loop runs in constant stack.
