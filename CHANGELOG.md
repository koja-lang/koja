...
- The time types. `DateTime` is now `Timestamp` and counts microseconds since the Unix epoch. `Duration` counts nanoseconds. The constructors and accessors spell out their unit: `Duration.from_seconds`, `from_milliseconds`, `from_microseconds`, `from_nanoseconds`, and the matching `as_*` accessors, which truncate. The new `Instant` reads the monotonic clock and is the type for elapsed time: `started = Instant.now()` then `started.elapsed()`. To migrate, `DateTime.now().timestamp_millis()` becomes `Timestamp.now().as_milliseconds()`, `Duration.from_secs` and `from_millis` become `from_seconds` and `from_milliseconds`, `millis()` becomes `as_milliseconds()`, and any two wall-clock reads subtracted to time something become one `Instant`. A stale `DateTime` gets a compile error that names the new type.
