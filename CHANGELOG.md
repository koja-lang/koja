...
- `koja new` scaffolds `test/app_test.koja` as a `test` block with one `assert`, and the generated `AGENTS.md` describes `test` blocks and `assert` instead of `@test` functions.
