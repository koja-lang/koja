...
- **Internal**: 137 stdlib intrinsic stubs across `lib/std` and `lib/net` migrated from `panic("intrinsic")` placeholder bodies to `@intrinsic` annotations on bodyless declarations. Codegen routes annotation-keyed dispatch instead of name-pattern matching for source-declared intrinsics.
