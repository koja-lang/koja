//! Minimal blocking HTTP/1.1 static file server for `expo doc
...
/// `expo doc serve` walk every port.
...
/// `expo doc serve` exits with a friendly message rather than a
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ServeError::DocRoot(p, e) => {
                write!(
                    f,
                    "cannot serve {}: {e} (try running `expo doc` first)",
                    p.display()
                )
            }
