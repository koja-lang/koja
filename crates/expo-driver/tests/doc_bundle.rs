//! Smoke tests for `expo doc` and `expo doc --project-only`.
//!
//! Spins the compiled `expo` binary against a tiny fixture
...
fn run_doc(cwd: &Path, args: &[&str]) {
    let output = Command::new(expo_bin())
        ...
        .current_dir(cwd)
        .output()
        .expect("failed to run expo doc");
    assert!(
        output.status.success(),
        "expo doc {:?} failed (stderr={})",
        args,
...
fn spawn_serve(cwd: &Path, port: u16) -> Child {
    Command::new(expo_bin())
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("failed to spawn expo doc serve")
}
