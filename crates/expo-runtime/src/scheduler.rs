...
/// Returns a mutable reference to the global scheduler, initialising it on
/// first access.
...
// ---------------------------------------------------------------------------
// Runtime intrinsics (C ABI, alpha-sorted)
// ---------------------------------------------------------------------------

/// Runs the scheduler loop until all processes have finished or a deadlock
/// is detected. Called after main returns to drain remaining work.
#[unsafe(no_mangle)]
...
/// Blocks the current process until a message arrives in its mailbox.
/// Returns a pointer to the received message payload.
#[unsafe(no_mangle)]
...
/// even if no message has arrived (returns null in that case).
#[unsafe(no_mangle)]
...
/// Delivers a message to another process's mailbox, unblocking it if it
/// was waiting on a receive.
///
/// # Safety
...
/// Spawns a new lightweight process that will execute `fn_ptr` with a copy
/// of the provided initial state. Returns the new process ID.
///
/// # Safety
