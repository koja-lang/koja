...
//! `CPtr<UInt8>`: `to_string` reads the v1 length-prefixed Expo
...
/// Header offset for the length-prefixed Expo string / binary ABI
/// (`[i64 bit_length][payload…]`). Matches
/// [`expo_runtime::util::STRING_HEADER_SIZE`].
        ...
        return Err(RuntimeError::Unsupported {
            detail: format!(
                "CPtr.to_string: source header carries negative bit_length {bit_length}; \
                 buffer does not look like an Expo string payload"
            ),
        });
    }
