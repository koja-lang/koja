...
use koja_ast::ast::{Item, StringPart};
    ...
    for file in &pkg.files {
        for item in &file.items {
            ...
            for member in &block.members {
                let koja_ast::ast::ImplMember::Function(function) = member else {
                    continue;
                };
                ...
                let koja_ast::ast::Statement::Expr(expr) = &body[0] else {
                    panic!("format body must be a single expression statement");
                };
                let koja_ast::ast::ExprKind::String { parts, .. } = &expr.kind else {
                    panic!(
...
fn type_expr_head_eq(te: &koja_ast::ast::TypeExpr, expected: &str) -> bool {
    match te {
        koja_ast::ast::TypeExpr::Named { path, .. }
        | koja_ast::ast::TypeExpr::Generic { path, .. } => {
    ...
    for file in &pkg.files {
        for item in &file.items {
            ...
            for member in &block.members {
                ...
                let koja_ast::ast::Statement::Expr(expr) =
                    &function.body.as_ref().expect("body")[0]
                else {
                    panic!("expected expr body");
                };
                let koja_ast::ast::ExprKind::Match { arms, .. } = &expr.kind else {
