...
//!   outer struct of shape `{ i64, [M x i64] }`, where `M` is the
...
//!   union slot lowers to alloca-the-outer + store the `i64` tag
//!   word at field 0 + store the typed payload through field 1,
//!   then load the populated outer back as the SSA result.
//! - **Typed-binding lowering**: a `p: Member -> body` arm in a
//!   `match` over a union subject lowers to a tag-word load
//!   truncated to `i8` + `==` compare + conditional branch into a
//!   body block whose head reads the payload back through field 1,
//!   typed as the member.
//!
...
fn union_outer_struct_emits_with_payload_buffer() {
    // The union outer is a named struct over `{ i64, [M x i64] }`
    // where `M` is the byte size of the largest member rounded up
    // to whole words. With two single-pointer-shaped members
    // (`{ ptr }` for `Post` / `Comment`), `M` is 1 on 64-bit hosts.
    let source = "
        struct Post
          title: String
        end
    ...
    assert_contains(
        &ir_text,
        "%Union_TestApp.Comment_or_TestApp.Post = type { i64,",
    );
    // One-word payload buffer, pinned only on 64-bit hosts where
    // single-ptr fields measure 8 bytes. The IR layout pass uses
    // host pointer width.
    #[cfg(target_pointer_width = "64")]
    assert_contains(
        &ir_text,
        "%Union_TestApp.Comment_or_TestApp.Post = type { i64, [1 x i64] }",
    );
}
...
fn unit_tuple_union_payload_matches_llvm_tuple_size() {
    ...
    assert_contains(
        &ir_text,
        "%\"Union_Tuple_$Unit,Int64$_or_Bool\" = type { i64, [2 x i64] }",
    );
}
...
fn union_wrap_emits_tag_store_then_payload_store() {
    ...
    // alloca-the-outer -> GEP field 0 -> store the i64 tag word ->
    // GEP field 1 -> store the typed payload -> load the outer back
    // as the SSA result. Substring-pinned on the tag store
    // (`store i64`) and the payload-pointer GEP into field 1.
    let source = "
        struct A
          x: Int
        end
    ...
    // the tag store materializes `i64 1`. `A` would store `i64 0`.
    assert_contains(
        &ir_text,
        "store i64 1, ptr %Union_TestApp.A_or_TestApp.B_tag_ptr",
    );
...
fn typed_binding_arm_emits_tag_load_compare_branch_and_payload_load() {
    ...
    let ir_text =
        emit_script_llvm_ir(&script, APP_NAME).expect("emit_script_llvm_ir should succeed");
    // Tag load reads the word and truncates to the `i8` the IR
    // contract carries. The per-arm compare follows.
    assert_contains(
        &ir_text,
        "load i64, ptr %Union_TestApp.A_or_TestApp.B_tag_ptr",
    );
...
fn struct_field_union_typechecks_and_emits() {
    ...
    assert_contains(&ir_text, "%Union_TestApp.Cat_or_TestApp.Dog = type { i64,");
