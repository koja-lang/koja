...
fn alias_is_file_private() {
    // `a.expo` defines an alias; `b.expo` (sister file in the same
    // package) tries to use the alias's local name. Aliases are
    // file-private, so `b.expo` should fail to resolve `Hasher`.
    ...
    let failure = result.expect_err("sister file should not see a.expo's alias");
    let messages = diagnostic_messages(&failure);
    assert!(
        messages
            .iter()
            .any(|m| m.contains("Hasher") || m.contains("does not recognize")),
        "expected unknown-Hasher diagnostic in b.expo, got: {messages:?}",
    );
}
