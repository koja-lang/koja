...
use expo_ast::ast::{Diagnostic, Item};
...
fn diagnose_alias_cycles(
    ...
    let mut alias_ids: Vec<(GlobalRegistryId, Identifier, expo_ast::span::Span)> = Vec::new();
    for pkg in packages {
        for file in &pkg.files {
            for item in &file.items {
                let Item::TypeAlias(alias) = item else {
    ...
    let mut cycling: Vec<(GlobalRegistryId, Identifier, expo_ast::span::Span)> = Vec::new();
...
fn type_references_alias(
    ...
    match ty {
        ...
        ResolvedType::Named { type_args, .. } => type_args
            .iter()
            .any(|arg| type_references_alias(arg, registry, seen)),
        ResolvedType::Anonymous(expo_ast::identifier::AnonymousKind::Function { params, ret }) => {
            params
                .iter()
