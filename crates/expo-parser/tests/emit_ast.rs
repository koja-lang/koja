//! End-to-end snapshot tests for [`expo_ast::format_file`], the
//! compact tree printer used by `expo parse --emit-ast` and `expo
...
use expo_parser::{ParseMode, SourceFile, parse_file};

fn format_snippet(source: &str) -> String {
    let parsed = parse_file(
        SourceFile {
            package: "TestApp".to_string(),
            path: PathBuf::from("/tmp/demo.expo"),
    ...
    expo_ast::format_file(&parsed.ast)
}
...
fn addition_literal() {
    ...
    let expected = r#"File TestApp "/tmp/demo.expo" @1:1-2:1
  body
    Binary Add @1:1-1:6
...
fn function_add() {
    ...
    let expected = r#"File TestApp "/tmp/demo.expo" @1:1-4:1
  items
