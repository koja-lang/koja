//! Externs declared in `lib/global/src/system.expo`.
//!
//! Wrappers over `expo_runtime::system` for env-var, cwd, and
...
//!
//! The pointer-returning calls (`expo_cwd`, `expo_get_env`,
//! `expo_hostname`) hand back a runtime-allocated payload that
//! `CPtr<UInt8>.to_cstring` walks. Eval just round-trips the raw
//! pointer through [`Value::CPtr`]; the runtime side owns the
//! storage layout.
//!
//! `expo_get_env` returns a null pointer when the requested variable
...
unsafe extern "C" {
    fn expo_cwd() -> *const u8;
    fn expo_get_env(key_ptr: *const u8) -> *const u8;
    fn expo_hostname() -> *const u8;
    fn expo_set_env(key_ptr: *const u8, val_ptr: *const u8);
}

pub(super) fn cwd(args: &[Value]) -> Result<Value, RuntimeError> {
    if !args.is_empty() {
        return Err(type_mismatch("expo_cwd", "()", args));
    }
    let ptr = unsafe { expo_cwd() };
    Ok(Value::CPtr(ptr as *mut u8))
}

pub(super) fn get_env(args: &[Value]) -> Result<Value, RuntimeError> {
    let [Value::CPtr(key)] = args else {
        return Err(type_mismatch("expo_get_env", "(key: CPtr<UInt8>)", args));
    };
    let ptr = unsafe { expo_get_env(*key as *const u8) };
    Ok(Value::CPtr(ptr as *mut u8))
}

pub(super) fn hostname(args: &[Value]) -> Result<Value, RuntimeError> {
    if !args.is_empty() {
        return Err(type_mismatch("expo_hostname", "()", args));
    }
    let ptr = unsafe { expo_hostname() };
    Ok(Value::CPtr(ptr as *mut u8))
}
...
pub(super) fn set_env(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    unsafe { expo_set_env(*key as *const u8, *val as *const u8) };
    Ok(Value::Unit)
}
