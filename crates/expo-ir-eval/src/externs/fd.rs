//! Externs declared in `lib/global/src/fd.expo`.
//!
//! Three families:
//!
//! - **Plain fd I/O** (`expo_fd_close` / `expo_fd_read` / `expo_fd_write`)
//!   — call straight into [`expo_runtime::fs`] over libc so eval and
//!   the LLVM backend observe the same kernel return values.
//! - **File-path operations** (`expo_file_*`) — wrap the runtime's
//!   path-based helpers; the runtime owns null-termination and CStr
//!   parsing on the C side.
//! - **Actor-coupled I/O** (`expo_io_block`, `expo_rt_watch_fd`,
//!   `expo_rt_unwatch_fd`) — register here so dispatch routes them,
//!   but they require an initialized scheduler / reactor in the
//!   runtime; calling them from a plain `expo eval` panics inside
...
unsafe extern "C" {
    fn expo_fd_close(fd: i32) -> i32;
    fn expo_fd_read(fd: i32, count: i64) -> *const u8;
    fn expo_fd_write(fd: i32, data_ptr: *const u8, data_len: i64) -> i64;
    fn expo_file_delete(path_ptr: *const u8) -> i64;
    fn expo_file_exists(path_ptr: *const u8) -> i64;
    fn expo_file_open(path_ptr: *const u8, mode: i64) -> i32;
    fn expo_file_read_all(path_ptr: *const u8) -> *const u8;
    fn expo_file_rename(src_ptr: *const u8, dst_ptr: *const u8) -> i64;
    fn expo_file_write_all(path_ptr: *const u8, content_ptr: *const u8) -> i64;
    fn expo_io_block(fd: i32, readable: i64);
    fn expo_rt_unwatch_fd(fd: i32);
    fn expo_rt_watch_fd(fd: i32, interest: i64);
}

pub(super) fn fd_close(args: &[Value]) -> Result<Value, RuntimeError> {
    let [Value::Int(fd)] = args else {
        return Err(type_mismatch("expo_fd_close", "(fd: Int32)", args));
    };
    let ret = unsafe { expo_fd_close(*fd as i32) };
...
pub(super) fn fd_read(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let ptr = unsafe { expo_fd_read(*fd as i32, *count) };
...
pub(super) fn fd_write(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let ret = unsafe { expo_fd_write(*fd as i32, *data as *const u8, *len) };
    Ok(Value::Int(ret))
}
...
pub(super) fn file_delete(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let ret = unsafe { expo_file_delete(*path as *const u8) };
    Ok(Value::Int(ret))
}
...
pub(super) fn file_exists(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let ret = unsafe { expo_file_exists(*path as *const u8) };
    Ok(Value::Int(ret))
}
...
pub(super) fn file_open(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let ret = unsafe { expo_file_open(*path as *const u8, *mode) };
    Ok(Value::Int(ret as i64))
}
...
pub(super) fn file_read_all(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let ptr = unsafe { expo_file_read_all(*path as *const u8) };
    Ok(Value::CPtr(ptr as *mut u8))
}
...
pub(super) fn file_rename(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let ret = unsafe { expo_file_rename(*src as *const u8, *dst as *const u8) };
    Ok(Value::Int(ret))
}
...
pub(super) fn file_write_all(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let ret = unsafe { expo_file_write_all(*path as *const u8, *content as *const u8) };
    Ok(Value::Int(ret))
}
...
pub(super) fn io_block(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    unsafe { expo_io_block(*fd as i32, *readable) };
    Ok(Value::Unit)
}

pub(super) fn rt_unwatch_fd(args: &[Value]) -> Result<Value, RuntimeError> {
    let [Value::Int(fd)] = args else {
        return Err(type_mismatch("expo_rt_unwatch_fd", "(fd: Int32)", args));
    };
    unsafe { expo_rt_unwatch_fd(*fd as i32) };
    Ok(Value::Unit)
}
...
pub(super) fn rt_watch_fd(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    unsafe { expo_rt_watch_fd(*fd as i32, *interest) };
    Ok(Value::Unit)
}
