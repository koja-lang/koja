...
Consumes the sealed `IRProgram` / `IRScript` produced by `expo-ir`.
...
The input is **always sealed**: `expo-ir::lower_program` / `lower_script`
...
upstream and panic in `expo-ir::seal`, never surface here.
...
`Value` mirrors `expo-ir::ConstValue` plus the runtime-only variants needed
