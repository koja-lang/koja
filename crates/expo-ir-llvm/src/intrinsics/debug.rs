...
//! (`expo_format_*`) that returns a freshly-allocated Expo string
...
//! sign-extend to `i64` and route through `expo_format_i64`;
//! unsigned (`UIntN`) zero-extend to `i64` (`u64` ABI-wise) and
//! route through `expo_format_u64`. `Bool` zero-extends through
...
/// `Bool` widens through `expo_format_bool(i64)`. The shared
...
/// 64-bit ABI and route through `expo_format_i64` /
/// `expo_format_u64`. The runtime side renders signed and unsigned
