...
fn parse_file_threads_path_and_package_into_ast() {
    ...
    assert_eq!(parsed.path, PathBuf::from("src/main.koja"));
}
...
fn parse_program_preserves_input_order() {
    let sources = vec![
        SourceFile {
            package: "p".to_string(),
            path: PathBuf::from("zeta.koja"),
            source: "fn z\n  1\nend\n".to_string(),
        },
        SourceFile {
            package: "p".to_string(),
            path: PathBuf::from("alpha.koja"),
            source: "fn a\n  1\nend\n".to_string(),
        },
        SourceFile {
            package: "p".to_string(),
            path: PathBuf::from("mu.koja"),
            source: "fn m\n  1\nend\n".to_string(),
        },
    ];
    ...
    assert_eq!(
        program.order,
        vec![
            PathBuf::from("zeta.koja"),
            PathBuf::from("alpha.koja"),
            PathBuf::from("mu.koja"),
        ]
    );
}
...
fn parse_program_iter_walks_in_order() {
    ...
    assert_eq!(
        names,
        vec![PathBuf::from("b.koja"), PathBuf::from("a.koja")]
    );
}
...
fn parse_program_get_finds_files_by_path() {
    let sources = vec![SourceFile {
        package: "p".to_string(),
        path: PathBuf::from("solo.koja"),
        source: "fn solo\n  1\nend\n".to_string(),
    }];
    let program = parse_program(sources, ParseMode::File);
    assert!(program.get(&PathBuf::from("solo.koja")).is_some());
    assert!(program.get(&PathBuf::from("missing.koja")).is_none());
}
