...
- `control/` -- Control flow, split by family: `conditional` (`if`, plus the removal diagnostic for `unless`), `loops` (`for`/`while`/`loop`), `match_arms` (`match`/`cond`/`receive`). Arm bodies use statement parsing and reserve an unframed `x -> ...` for the next arm
