...
/// position in declaration order, and backends use it as the LLVM
/// struct field index for GEPs.
...
/// mangled name shape an [`crate::IRFunction`] uses, and `fields` is the
/// declaration-order field list. Empty `fields` is legal (matches
/// `struct Foo / end` shape). Generic decls never appear here, as
...
/// `index` selects the declared field by position, and `value` is the
