...
`checked` is a sealed [`CheckedProgram`] from `koja-typecheck`. `entry` is the
...
explicit lowering rules and a 1:1 emit step in `koja-ir-llvm` /
`koja-ir-eval`. New instructions land as new features create pressure
