...
The destination design for Koja's process supervision: crash isolation,
failure detection (monitors), guaranteed teardown, a global name registry,
and an OTP-shaped `Supervisor`. This is the ROADMAP Phase 5 A2 "design
first" deliverable — the gate that must close before any supervision code
lands. It also folds in the runtime-level half of A3 (`Process.register` /
`whereis`), because that primitive depends on the same `Process.Identifier`
type and `notify_exit` death path this doc owns.
...
Per-crash log-level control (quieting routine supervised restarts) is
deferred to a Phase 7 `logging` package; pre-1.0, a crash loop is loud,
which is acceptable and arguably desirable.
...
- [ROADMAP.md](ROADMAP.md) — Phase 5 A2 (supervision) and A3 (discovery);
  this doc is A2's "design first" gate and the runtime half of A3.
