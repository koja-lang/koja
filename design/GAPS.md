...
## Universal `Equality` on bare type parameters

Found 2026-08-23 while updating `auth_manager` for Koja 0.18, narrowed
2026-09-02 with issue #94. The derivation policy is now defined:

- A derived `Equality` for a generic type is conditional,
  `impl Equality for Option<T: Equality>`, the same shape the stdlib
  writes for `List`.
- A type whose field or payload types do not all satisfy `Equality`
  does not derive it. The compiler retracts the derived impl at lift
  with no diagnostic. `==` on the type then reports "does not implement
  `Equality`" with a hint that names the blocking field. Types with a
  direct function or union field lost their derived `Equality` under
  this rule, since the old derive skipped those fields and compared
  distinct values as equal.
- Calling a protocol method on an instantiation that fails the impl's
  condition, such as `List<fn () -> Int>.equals?`, is a typecheck
  error.
- Derived `Debug` stays unconditional. IR renders function receivers as
  `...`, so it never reaches a missing body.

One hole remains. `Equality` is still in `UNIVERSAL_PROTOCOLS`, so a
bare `T` with no declared bound satisfies it. `fn same<T>(a: T, b: T)
-> Bool a.equals?(b) end` typechecks and accepts function arguments,
and IR lowering panics on the function receiver.

The intended end state (decided 2026-09-02) closes the hole from the
other side: functions become `Equality` values with the BEAM rule. Two
function values are equal when they come from the same function or
closure expression and their captured values are equal. `&f/1 == &f/1`
is true by name, `f == f` is true, and two closures from one site are
equal exactly when their captures are. Captures are by-value snapshots,
so the pair (site, captures) fully determines a closure's behavior and
the rule never observes allocation. Two textually identical closures at
different sites compare unequal, which is a false negative and the
same choice the BEAM makes. Content-addressed equality (Unison) was
considered and rejected: it couples equality to the normalizer, so a
compiler upgrade could flip a comparison with no source change.

Implementation notes for that work:

- Compare an explicit compiler-assigned site id, not the code pointer.
  LLVM merges identical bodies, which is the Rust `fn` pointer footgun.
  The compiler must never merge distinct expressions where equality can
  observe it.
- The id is post-monomorphization. `make(1) == make("a")` typechecks
  when both return `fn () -> Int` and the bodies differ.
- Each closure block carries a synthesized `equals?` over its own
  captures (one extra word, or a side table keyed by id), since the
  caller only knows the function type, not the layout. `Hash` follows
  the same shape so `Set<fn ...>` agrees with `==`.
- A captured value that is not `Equality` (today, a union) makes the
  closure not `Equality`, the same conditional rule the struct derive
  applies.
- The retraction rule above stays in force until then, so types with
  function fields go from "does not compare" to "compares" with no
  breaking step.

A smaller sibling: derived `Debug` skips a field whose type is spelled
as a union, but not a field whose type is an alias to a union
(`type Pet = Cat | Dog`, then `pet: Pet`). The generated `format` body
then reports "cannot call method on union type" at the struct
declaration. Resolving the alias in the derive filter closes it.

**Fix path:** give function values site-plus-captures `Equality` as
above, then drop the function carve-outs in `type_supports_equality`
and the IR receiver check. Resolve aliases in the `Debug` derive
filter.
