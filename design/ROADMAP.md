...
- **Tail call optimization**: the `move self -> Self` pattern naturally encourages recursion (`self.step().run()`), but without TCO each recursive call pushes a new stack frame. The self-hosted lexer exhausted a 2MB stack on real input -- each `run()` frame carries the full `Lexer` struct (~300-500 bytes), so even a few thousand tokens overflows. This makes the language's core idiom unsafe for production use. Fix: detect self-recursive tail calls in codegen and emit LLVM `musttail` (requires matching signatures and calling conventions) or rewrite to a loop at the IR level. Alternatively, detect and warn on non-tail-recursive `move self -> Self` methods. Without TCO, users must manually rewrite natural recursion into loops, which defeats the language's functional style.
