...
- ~~**Tail call optimization**~~: **Done.** Self-recursive `move self -> Self` methods emit LLVM `musttail` in tail position (implicit returns and explicit `return`). Eliminates stack growth for the language's core recursive idiom. General TCO (mutual recursion, arbitrary tail calls) remains future work.
