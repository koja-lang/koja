...
- **`Pid` + `ExitSignal` + monitors (P2) -- in progress.** Stdlib types landed: bare `Pid` (the type-erased identity, distinct from `Ref<M, R>`; `Ref.pid()` widens — named `Pid` over `Process.Identifier` per the SUPERVISION.md naming decision), `Process.ExitSignal`, `Process.MonitorRef`, `Process.CrashInfo`, with `Process.ExitReason` widened to `Crashed(CrashInfo)`. Remaining: `Process.monitor`/`demonitor` delivery (staged off-lock from `notify_exit`), the `ExitSignal`-in-`M` typecheck rule, and `ReplyTo.send -> Result<(), ReplyError>`. _(`ProcessTable::notify_exit(pid, reason)` is still a no-op seam for monitor delivery; the death-site reason (`Normal`/`Shutdown`/`Killed`/`Crashed`) is recorded.)_
