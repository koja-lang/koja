...
- Graceful SIGTERM drain _(done)_ -- a `SIGTERM` puts the runtime in a draining mode: new spawns are refused (`spawn` yields a ref to an already-dead process) and a grace deadline is armed (`KOJA_GRACE_MS`, default 30s to match k8s). The `Shutdown` signal is still delivered, so a lifecycle-aware program can stop itself first; once the deadline passes, the driver force-kills any straggler (`Killed`) so the runtime tears down. The mode + deadline live on the `koja-runtime-core` `ProcessTable`, so both backends inherit it. This is the mechanism (deadline-bounded teardown); supervised draining that waits on in-flight work needs supervision (A2).
- Timer wheel for timeouts, intervals, deadlines. _(timeouts + deadlines done — a single hashed wheel + overflow heap in `koja-runtime-core` unifies `send_after` deliveries and receive/call deadlines behind the existing `ProcessTable` API. Periodic intervals (`Ref.send_interval`) still open: sound re-arming of a payload that owns ref-counted heap needs an acquire/clone glue codegen path the runtime does not have yet — `send_after` only ever receives release glue.)_
- Preemption + priority _(done)_ -- the `koja-ir` `yield_checks` pass inserts a `YieldCheck` at every loop back-edge and before each tail call; it spends one reduction from the running process's per-quantum budget (`Priority::budget`, granted by `Low` / `Normal` / `High` and reset on each schedule) and re-queues the process when the budget runs out. LLVM lowers it to `koja_rt_yield_check`; the cooperative interpreter routes it through `scheduler::reduce`. This is what lets the busy-wait `memory/` fixtures run under both backends. **Yield-check overhead is doubly important** since WASM has no native preemption -- coop yields are the only defense against starvation there.
