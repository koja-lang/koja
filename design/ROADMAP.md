...
- **Block expression return type inference**: `match`, `if/else`, and `cond` expressions always infer as `Type::Unknown`, so the implicit return type check is bypassed when a function's last expression is a block. A function declared `-> Option<String>` whose body ends with a `match` that only does assignments passes `expo check` but panics in codegen. Fix requires inferring result types from arm bodies (unify last expressions across all arms) for `match`, `if/else`, and `cond`.
- **Mandatory `return` in functions** (design investigation): require explicit `return` in all non-Unit functions (closures exempt -- last expression is the return value, matching Kotlin/Swift). Motivations: (1) eliminates the block expression return type inference bug above -- codegen always sees explicit `return`, never an implicit tail expression; (2) prevents accidental return type changes when appending side-effect code to a function body; (3) resolves the stylistic mismatch where guard clauses use `return` but the "normal" path relies on implicit returns. Rule: functions require `return`, closures don't. Bare trailing expressions in functions with a declared return type become a compile error.
...
- **`List.get` and `String.get` panic on OOB**: `List.get(i)` and `String.get(i)` panic instead of returning `Option<T>`. `Map.get(k)` already returns `Option<V>`. Decision: unify all `get` to return `Option`, remove `String.at()` (redundant). Previously blocked on type resolution -- `collect` couldn't resolve cross-module generic return types like `Option<String>`. The two-phase collection refactor (`GlobalNames` + `collect_all_names`) resolved this blocker; implementation is now straightforward (signature + codegen changes only)
