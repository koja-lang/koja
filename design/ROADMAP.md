...
- **Compiler architecture** -- destination spec in [COMPILER-NORTHSTAR.md](COMPILER-NORTHSTAR.md) (four-phase linear pipeline, sealed `IRProgram`, package-granular incremental, single `TypeIdentifier` for all bound names). Predecessor docs: [archive/20260502-EXPOIR-ROADMAP.md](archive/20260502-EXPOIR-ROADMAP.md) (live ExpoIR roadmap), [archive/20260502-COMPILER.md](archive/20260502-COMPILER.md) (earlier self-hosted compiler sketch), and [archive/20260427-EXPOIR.md](archive/20260427-EXPOIR.md) (original SIL-style design and Wave 1-17 history).
...
Phase 1 (bootstrap compiler) and Phase 2 (core language) are complete. Phase 3 (language surface + runtime maturity) is complete. Phase 4 (stdlib + ecosystem; runtime + reliability) is complete. The full build history with detailed implementation notes is preserved in [archive/20260318-ROADMAP.md](archive/20260318-ROADMAP.md), [archive/20260330-ROADMAP.md](archive/20260330-ROADMAP.md), and [archive/20260518-ROADMAP.md](archive/20260518-ROADMAP.md). Phase 5 is in progress; it introduces a new top-level `expoc/` subproject (sibling to `crates/`, `lib/`, `tests/`, `examples/`) housing the Expo-written self-host pipeline, starting with the parser.
...
Phase 4 made Expo useful for real programs (Track A) and concurrency production-grade (Track B). Both tracks landed; the convergence point (an HTTP service composed from stdlib primitives running on the production scheduler) is hit by `examples/demo_api`.

### Track A: Stdlib + Ecosystem -- done

- **Test runner** -- `expo test` discovers `@test`-annotated functions, generates a synthetic harness, compiles and runs.
...
Dual entry mode landed: `entry = "App"` (PascalCase) spawns a `Process` impl; `entry = "main"` (lowercase) retains `fn main`. Both coexist. Exit-code mapping (`Normal -> 0`, `Shutdown -> 1`) and argv passing (when `C = List<String>`) wired through. The `expo new` scaffolding flip and full `fn main` retirement are tracked in Phase 7.
...
The first contained self-host milestone (parser via a new top-level `expoc/` subproject), the runtime completion that supervises crashes and drains gracefully, and the interactive REPL that turns `expo shell` into a real development surface. Convergence: `demo_api` becomes supervised, parsed by `expoc`, and interactively developable.

```mermaid
flowchart LR
  TrackA["Track A: expoc parser self-host"] --> Conv["Convergence: supervised demo_api, parsed by expoc, developed in expo shell -S ."]
  TrackB["Track B: Runtime + Supervision"] --> Conv
  TrackC["Track C: expo shell -S ."] --> Conv
```

### Track A: Self-host the parser via `expoc/`

A new top-level `expoc/` subproject in the monorepo, alongside `crates/`, `lib/`, `tests/`, `examples/`. First-class subproject -- Expo source code that compiles to a standalone binary the Rust pipeline can delegate to. Eventually graduates to the canonical compiler when feature-complete and relocates to its own repo at that cutover.
...
### Track C: Interactive workflow (`expo shell -S .`)

Goal: `expo shell` graduates from scratch evaluator to project development surface. Stays on the in-process Rust parser during Phase 5 to protect REPL latency; routing through `expoc` is a later decision.

- `expo shell -S .` loads a project (`expo.toml`, deps, all package sources) into the interpreter's session state.
...
- Tab completion for module names, functions, types, and variables in scope (reuse the `expo-lsp` completion plumbing where it generalizes).
- Process inspection: list spawned processes (pid, state, mailbox depth), dump a mailbox, step a single message.
- **Done when**: `demo_api` can be developed end-to-end inside `expo shell -S .` -- start the server, exercise it from a sibling REPL session, inspect running process state, edit code, restart.
...
- **Self-host-parseable** -- builds byte-identically via `expo build --parser=expoc demo_api/` (Track A).
- **Interactively developable** -- the whole project can be loaded and iterated on inside `expo shell -S .` (Track C).

### Risks

- **Language gaps during parser port** -- `expoc/src/parser.expo` is the first non-trivial Expo program. Expect to discover language shortcomings; treat each as an opportunity to harden pre-1.0 rather than as scope creep.
...
- **Subprocess latency for interactive surfaces** -- protected by keeping `expo shell` and `expo-lsp` on the in-process Rust parser during Phase 5.
...
- Documentation & discovery polish (doctests, doc search, prose pages, `expo query`, `expo guide`)
- LSP finishing pass (inlay hints, multi-module diagnostics)
- `expo new` scaffolding flip + retire `fn main`
- `HTTP.Server` wrapper
...
- Validation projects (auth-manager-expo, second project)
...
- Continued `expoc` self-host (typecheck port, IR port, backend port, eventually retire Rust pipeline)
- Interpreter port to Expo (considered as Phase 5 Track A; pivoted to parser-first because tighter blast radius)
- LLVM backend port to Expo (requires C FFI Phase 3, additive once 1.0 ships)
...
WASM-before-1.0 commitment plus the cross-compile infrastructure that makes "Expo runs anywhere" real. Two tracks, single convergence: from any tier-1 host, `expo build --target=<any-tier-1-target>` cross-compiles correctly; specifically `expo build --target=wasm32-wasi demo_api/` produces a `.wasm` that runs under `wasmtime`.
...
- **Runtime split** -- `expo-runtime-core` (target-agnostic scheduler skeleton, conforming to the Phase 5 B1 scheduler protocol) plus per-target adapters (`expo-runtime-posix`, `expo-runtime-wasi`, eventually `expo-runtime-browser`).
...
- **Done when**: `expo build --target=wasm32-wasi demo_api/` produces a `.wasm` that `wasmtime demo_api.wasm` runs correctly (responses returned, lifecycle handled where applicable).

### Track B: Cross-compile distribution

- **Per-target static deps** -- generalize `include_bytes!` embedding to a `HashMap<TargetTriple, &'static [u8]>` for `libexpo-runtime-<target>.a`, `libcrypto-<target>.a` (where applicable), etc.
- **`--target=<triple>` plumbing** -- driver flag, target spec table, per-target linker invocation (linker selection, output format, ABI flags). macOS deployment-target / triple bookkeeping generalized from current single-target form in `crates/expo-driver/src/link.rs` and `crates/expo-ir-llvm/src/object.rs`.
- **CI matrix** -- release builds per host platform (macOS-aarch64, macOS-x86_64, Linux-x86_64, Linux-aarch64 as initial tier-1). Each host's `expo` ships with embedded blobs for every tier-1 target.
- **GitHub Releases automation** -- tagged release pushes per-host tarballs (`expo-v1.x.y-<host>-<arch>.tar.gz`); `brew tap`, `asdf plugin`, `mise install` formulas point at releases.
- **Optional: Zig-style bundled libc headers/sysroots** -- for fully hermetic `expo build --target=X` with zero additional setup on any host. Significant UX win, costs ~20-30 MB to the toolchain download. Default lean: include, since "Built to last" + "Approachable by default" both endorse.
...
- **Done when**: from a single host, `expo build --target=<any-tier-1-target>` cross-compiles correctly; `brew install expo` followed by `expo build --target=wasm32-wasi demo_api/` works with no further setup.
...
The polish, validation, and language-surface review that gates a published 1.0. Most of the "Considered for Phase 5, deferred" list lands here. Convergence: a signed, tagged, documented `expo 1.0` release with a stable spec and tier-1 platform binaries on GitHub Releases.

### Sub-deliverables

- **Documentation polish** -- doctests (code blocks in `@doc` strings compile + run as tests), client-side fuzzy search in `expo doc` output, prose pages from `docs/*.md`, clickable type cross-references in signatures.
- **CLI query/guide system** -- `expo query type <Type>` / `expo query module <pkg>` / `expo query protocol <Protocol>`; `expo guide <topic>` (prose guides on ownership, concurrency, FFI, protocols, testing, etc.).
- **LSP finishing pass** -- inlay hints for inferred types, multi-module cross-file diagnostics.
- **Ergonomic cleanup** -- `expo new` scaffolding generates a `Process` entry (not `fn main`); retire `fn main` as an entry mode; `HTTP.Server` wrapper landing in stdlib.
...
- **Validation projects** -- `auth-manager-expo` (the 17-file pseudocode reference) compiles and runs as a real service; one additional non-trivial project compiles cleanly (CLI tool or different microservice).
...
- **Done when**: `expo 1.0` is tagged, GitHub Releases artifacts exist for every tier-1 host × target combination, language spec is published, `auth-manager-expo` is running in some real-traffic context.
...
- **Continued `expoc` self-host** -- typecheck port, IR port, backend port; eventually retire the Rust pipeline. Phase 5 ships the parser; later phases pick up the next layer at their own pace.
- **Interpreter port to Expo** (`expo-ir-eval` → `expoc`) -- considered as Phase 5 Track A; pivoted to parser-first because tighter blast radius. Becomes a natural successor once the parser port stabilizes.
- **LLVM backend port to Expo** (`expo-ir-llvm` → `expoc`) -- requires C FFI Phase 3 (which lands in Phase 7). Additive once 1.0 ships.
...
| Phase    | Milestone                                                                                                                                                                                                                                                                |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 5A       | Parser self-host: top-level `expoc/` subproject, AST serialization format (versioned), `--emit-ast` + `--parser=<path>` flags, parser port, `just test-selfhost` parity sweep                                                                                            |
...
| 5C       | Interactive workflow: `expo shell -S .` project mode, inline `h` docs, tab completion, process inspection                                                                                                                                                                |
...
| Post-1.0 | Continued `expoc` self-host (typecheck → IR → backend), interpreter port to Expo, LLVM backend port to Expo, browser WASM, additional targets, 2.0 / additive evolution                                                                                                  |
