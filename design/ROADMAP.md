...
- **`Pid` + `ExitSignal` + monitors (P2) -- done.** Stdlib types: bare `Pid` (the type-erased identity, distinct from `Ref<M, R>`; `Ref.pid()` widens — named `Pid` over `Process.Identifier` per the SUPERVISION.md naming decision), `Process.ExitSignal`, `Process.MonitorRef`, `Process.CrashInfo`, with `Process.ExitReason` widened to `Crashed(CrashInfo)`. `ReplyTo.send` returns `ReplyTo.Delivery` (`Delivered`/`Expired`) with a linearizable check-and-deliver on both backends. `Process.monitor(pid)` / `Process.demonitor(ref)` land as protocol statics on both backends: monitors live in the core `ProcessTable`, exits stage `ExitNotice`s under the table lock and deliver off-lock (monitoring a dead pid delivers immediately), and an `ExitSignal` receive arm is synthesized in `koja-ir` elaborate so watchers handle it as an ordinary message. Typecheck enforces `ExitSignal`-in-`M` at `monitor` call sites.
- **Universal parenting + kill-cascade (P3) -- done.** Every spawn records the spawner as the new process's parent (`None` only for PID 1), exposed via the `Process.parent() -> Option<Pid>` protocol static on both backends. A process's death for any reason (return, crash, kill) stages a force-kill of its live children in `notify_exit`, settled by the adapters alongside `ExitSignal` delivery; the cascade is transitive, so an entire subtree tears down from any node's death. There is no detach: to outlive its creator, work must be spawned by a longer-lived process.
...
- **Ergonomic cleanup** -- ~~`koja new` scaffolding generates a `Process` entry (not `fn main`) and retires `fn main` as an entry mode~~ (**done**). Add contextual `TupleLiteral<T>` conversion. ~~Migrate process envelopes, intrinsics, iterators, and stdlib call sites from `Pair<A, B>` to tuples~~ (**done**), then remove the compatibility `Pair` type. Land the `HTTP.Server` wrapper in stdlib.
