...
**Status: draft design (2026-07-30). Nothing in this document is
implemented.** It records the design direction and its rationale so the
eventual implementation plan has a fixed target. The shipped error
surface today is `Result` / `Option` plus `match`; see
...
function calling into two error domains declares
...
mechanism: unwinding with dynamic handler search, where control
transfers N frames with no mark at intermediate call sites. This design
has none of that. Every hop is marked (`try`), every hop is one frame,
and the mechanism is an ordinary `return` of an ordinary value.
...
same place from the exceptions side; Rust's `?` reaches it from the
values side.

Where this design goes further than Zig and Swift: `!` introduces **no
semantic seam**. `-> Connection ! Error` _is_
...
semantics, zero runtime machinery, no unwind tables (which matters for
the leaf-staticlib runtime and eventual self-hosting), and deterministic
...
An obvious sharpening was considered and rejected: drop `Result`
...
type; the ABI is a flag plus payload, the same ABI `!` lowers to), so
it is a road taken, not a strawman.

It fails because three situations force an outcome to be **data**, and
a channel is control flow — it cannot sit still:
...
2. **Collections and batch work.** `urls.map(u -> fetch_limits(u))` —
   with channel-only errors, the closure's failure must pass _through_
   `map`, so function types grow effect information and every
   higher-order function needs effect polymorphism (a Koka-scale
   type-system purchase, plus function coloring). With `Result`
   underneath, a fallible closure is an ordinary closure returning an
   ordinary value and `map` works today, monomorphized.
3. **Generic code over fallibility.**
   `fn retry<T, E>(op: fn () -> Result<T, E>, attempts: Int)` is
   writable with zero new machinery; abstracting over a primitive
   channel is again effect polymorphism.

Swift's history is the empirical confirmation: it shipped channel-only
and later added `Result` to the stdlib because the community kept
reinventing it for exactly these cases, and it needs glue
(`Result(catching:)`, `.get()`) to cross between the views. Zig's
storable error unions can't carry payloads. Koja's version makes the
crossing free because there is nothing to cross: `T ! E` is the
control-flow view and `Result<T, E>` is the data view of one type.
Signatures show the channel; values show the data.

A union return (`T | E`, no wrapper) was also considered: it collapses
...
`! E` may only appear in return-type position. `E` is any type usable
...
- Statement position discards the `Ok` payload but still propagates,
  replacing the inverted `Err -> return Err; Ok(_) -> ()` match.
...
`return` is, including `match` arms (arms may already diverge).
...
one rule where what executes is not literally what is written; it read
fine in every worked example, but it is the dial to revisit first if
the implicitness bothers reviewers (options: wrap only the trailing
expression, or require explicit `Ok` on `return`).
...
binds the error and must produce `T` or diverge (`return`, `fail`,
`Kernel.panic`). The whole expression has type `T`.
...
handler is a single expression — anything bigger stays a `match`. There
is deliberately no block form (`begin ... rescue`); a block-scoped
rescue is a handler search, which is the goto this design exists to
avoid. Ruby's postfix expression `rescue` (`x = risky rescue default`)
...
the existing "member widens automatically" coercion, applied at the
`try` / `fail` boundary:
...
explicit collapse point. Union widening makes mapping unnecessary;
...
`try` is `Result`-only. Absence is not an error until the error is
named, and `Option.ok_or(err) -> Result<T, E>` (a new one-line stdlib
method) is where that happens:

```koja
scheme = try parsed.scheme.ok_or(Error.InvalidUrl("URL must be absolute: " <> url))
```
...
Panics remain process crashes handled by supervision, never catchable
in-process. Java's deepest sin was conflating bugs
...
- Success type `()` kills the `Result.Ok(true)` trailer; a passing
  body just ends.

The runner contract: any `-> () ! E` with `E: Debug`. Plain
...
failure enum and helpers that are ordinary `! Failure` functions — no
runner magic. Extractors return the unwrapped value, which is what
...
enum Failure
  Missing(String)         # expected a present value; what was absent
...
The helpers dogfood the feature (`assert_ok` is one `rescue` line;
`assert_some` is `try option.ok_or(Failure.Missing(what))`). The
...
uniform (every statement that can end the test is marked); if it
grates, the dial is a compiler-known `assert` statement participating
in the channel natively. And without macros the helpers cannot report
the assertion's line — that needs a Swift-`#line`-style
caller-location intrinsic someday; until then the runner reports the
test name and `Failure` carries the rest. (The `assert()` builtin
mentioned in older notes does not exist in the shipped surface; the
name is free.)
...
- **Postfix `?` (Rust).** The `?` glyph is already double-booked:
  identifiers may contain it (`empty?()`) and the ternary uses it
...
- **Gleam-style `use`.** General CPS sugar; weak priors, reads
...
- **Combinators only (`map_err`, `or_else`).** Cheapest, but real
  protocol code is intermediate-binding-heavy; closure chains do not
  fix its shape. (`ok_or` is worth adding regardless; `map_err`
  already exists.)
...
- **Type-routed `return` instead of `fail`** (plain `return` picks the
  ...
  stdlib: `File.read -> Result<String, String>` — success and error
  are the same type, so no routing rule exists; the tag is semantic,
  not derivable. It would also give every return site two candidate
  ...
  channel exit, `fail` marks the entry — the symmetry is the answer to
  "isn't this exceptions?". Note the shape that motivates the question
  is a feature: in a `! E` function, `fail`s enumerate the failure
  exits, the trailing expression is the one success exit, and early
  `return` of a success goes rare (cache-hit shape) but not extinct.
...
to `Option` and custom enums — and can be revisited after this ships.
...
- **Rust**: great value-level `Result`, no anonymous unions (trait
  ...
  impls — the gap `thiserror`/`anyhow` fill.
...
- **Roc**: anonymous tag unions plus a propagation operator — the
  closest existing kit, pre-1.0.
- **Java checked exceptions**: the failed ancestor. Right idea (typed,
  declared channel); failed on propagation ceremony (edit every
  `throws` clause or wrap-and-rethrow) and subclass-only composition —
  exactly the two points `try` + union widening fix.

Koja's enabling conjunction: structural unions as coercion at declared
boundaries (not general subtyping, so local inference survives), cheap
RC-backed error payloads, monomorphization (unions concrete by
codegen), no exception legacy, and a separate crash channel so the
error channel never carries bugs. None of these were designed for error
handling; they cohere here anyway.
...
- **No new type, no ABI change.** `T ! E` desugars to `Result<T, E>`
  during typecheck; IR, both backends, and the runtime never see it.
  Old and new signatures interoperate in both directions — new code
  can `try File.read(path)` against the untouched stdlib, and
  match-based callers consume `! E` functions without knowing.
- **Existing bodies check unchanged.** Ok-wrapping is gated on the `!`
...
- **Migration is optional and incremental.** Signatures can flip to
  `!` file by file with zero downstream breakage. `ok_or` is a pure
  stdlib addition.

## Effect on `Result`'s combinators

The rule after this ships: **control flow uses `try` / `rescue` /
`fail`; combinators are for outcomes-as-data** (results held in
collections, returned by `Task.await`, stored in fields — places
propagation cannot reach).

That retires the railway group. `then` is monadic bind — a `then`
chain is do-notation with the names stripped out, the same
readability failure as a deeply stacked Elixir `with` — and `try` is
the flattened, named-bindings replacement. A corpus scan (2026-07-30)
found one real `Result.then` call site (a trivial `try` rewrite) and
zero real `map_err` call sites; the corpus had already voted.
**`Result.then` should be deleted with this feature.** (`Option.then`
stays until the deferred `try`-on-`Option` question is settled —
chained lookups have no substitute yet.)

The data-view group survives: `ok?` / `err?` (predicates for `filter`
and tests), `ok` / `err` (Option bridges), `or` (terse default,
symmetric with `Option.or`), `unwrap` (explicit crash-channel
crossing when failure is a bug; `rescue e -> Kernel.panic(...)` is
the with-context form), `map` (transforming stored results), and
`map_err` (the only way to convert the error type of a _held_
`Result` — `rescue` produces `T`, not `Result<T, F>`), the latter two
documented as data-view tools.
...
- **Exhaustiveness over unions.** `match` on `Result.Err` of a union
  ...
  idiomatic; fixing it should ride with or precede this feature.
...
- **Keyword impact.** `try`, `fail`, and `rescue` become reserved
  words (pre-1.0 breakage is acceptable; `rescue` also permanently
...
- **Display convention.** `T ! E` and `Result<T, E>` are two spellings
  ...
  declared return, `Result` elsewhere.
