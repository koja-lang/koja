> **Archived 2026-04-03.** Project system implemented. Unresolved type system ideas extracted to [TYPES.md](../TYPES.md).
...
```
import std.string
  filesystem: find std/string.koja
...
`import std.io` tells the driver to find and compile `std/io.koja`. The types
...
`ProjectConfig` struct via serde. No Koja parser or AST involved.
...
The dependency must have its own `koja.toml`. The resolver loads it, scans
...
- On `koja build` / `koja test` / `koja check`: look for `koja.toml` in the
  current directory.
- If found: parse it, extract config, set project root to the `koja.toml`
  directory.
- If not found: fall back to current behavior (entry file's parent as root).
- Module discovery: scan `src` dirs for `.koja` files to build the module graph.
...
- **Project**: `koja.toml` defines the build unit.
...
- **Multiple types per file**: a file can define multiple types. When you
  ...
  in practice?
- **Entry file**: the entry module (`main.koja`) may have top-level statements
...
- **Test runner**: `@test` annotated functions with `koja test`. Discovery
  scans `test` dirs from `koja.toml`. Implementation details deferred to
...
Koja can avoid `mapN` entirely by leveraging `JsonValue` as the
