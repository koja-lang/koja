...
> **Status 2026-06-09 — `fn main` removed.** The original proposal's entry-point story is now fully implemented: every compiled program's entry is a type implementing `Process` (named by `entry` in `koja.toml`), `.kojs` scripts cover the zero-ceremony case, and the `fn main` path (including standalone `.koja` execution and the `entry` casing trick) has been deleted from the compiler. The 2026-04-18 revision's one-extension idea (`fn main` as the script idiom in a single `.koja` file) is superseded — `.kojs` shipped and `fn main` did not survive as an entry shape. The package-as-type namespace ideas in that revision remain open for a future design pass.
...
The `entry` field in `koja.toml` names a type that implements `Process<C, M, R>`; codegen generates a C `main` that constructs and spawns it. The `fn main` path (and the lowercase/PascalCase casing trick) was removed on 2026-06-09 — `.kojs` scripts cover the entry-free case.
