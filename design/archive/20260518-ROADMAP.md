...
- ~~**Lifecycle event delivery**~~ -- OS signals captured via `sigaction` in `expo-runtime` and dispatched as `Lifecycle` variants (`Shutdown`, `Interrupt`, `Reload`) into the entry process's mailbox; `handle_signal` receives them. `SIGTERM -> Shutdown`, `SIGINT -> Interrupt`, `SIGHUP -> Reload`. Validated by `examples/demo_api`.

**Remaining:**

- **`expo new` scaffolding** -- update to generate Process entry instead of `fn main`.
- **Remove `fn main`** -- once `.exps` scripts are implemented, `fn main` becomes unnecessary and can be removed. (`.exps` is done; only the scaffolding flip + removal remain.)
