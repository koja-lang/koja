> **Archived 2026-04-03.** Superseded by [FNMAIN.md](../FNMAIN.md). Remaining open questions (panic behavior, supervision) merged into FNMAIN.md.
...
- `spawn_process.koja` -- Worker process handles `Greet`, `Add`, `Stop`
  messages via `cast`
- `call_roundtrip.koja` -- Calc process handles `Add`, replies via
  `ReplyTo<Int>`, caller uses `call` with timeout
- `cast_loop.koja` -- Counter process maintains state across `cast` messages
- `task.koja` -- `Task.async` / `Task.await` for one-off async work
...
Koja needs to support two very different use cases:
...
- **`koja run file.koja`** uses `fn main`. No Process protocol, no signal
  handling, no supervision. The program runs and exits. This is what you
  get when you write a standalone `.koja` file.

- **`koja new project_name`** scaffolds a project. The initial implementation
...
- **Library-only projects.** No entry point at all. `koja new --lib` or
  a flag in `koja.toml`.
...
per-OS-process, not per-Koja-process. Any Koja process can read them.
