...
A 10-crate Rust workspace that compiles Koja source to native binaries via LLVM:
...
- `koja-lsp` -- language server (diagnostics, formatting, hover, go-to-definition)
...
- Lightweight processes -- `spawn` creates a process, `receive` blocks for the next mailbox message, `Process<M>` is a typed handle with `send(msg: M)`. Message type `M` can be any type (primitives, structs, enums). Backed by `koja-runtime` cooperative scheduler.
...
Build a minimal Koja compiler in Rust that can compile trivial programs to native binaries via LLVM.
...
- ~~**Deliverable**: `koja check file.koja` reports type errors with clear messages~~
...
- ~~**Deliverable**: `koja build hello.koja` produces a native binary that runs~~
...
Make the compiler powerful enough to compile non-trivial programs with Koja's generics, ownership model, and structured concurrency.
...
- ~~The `&` symbol does not exist in Koja -- borrowing is implicit~~
...
- ~~`List<T>` iterator functions (`map`, `filter`, `any?`, `all?`) -- implemented as pure Koja code in `std.kernel`, not intrinsics. Enabled by intrinsic dispatch fallthrough (`EmitResult` enum) and codegen fix for generic type resolution in method bodies.~~
...
**Shipped**: lightweight processes via `spawn`/`receive`/`Process<M>`. A process runs a function on the cooperative scheduler, receives typed messages through a mailbox, and is identified by a `Process<M>` handle. Message type `M` can be any type (primitives, structs, enums). Backed by `koja-runtime`, a Rust static library providing a single-threaded cooperative scheduler with per-process mailboxes. String equality (`==`, `!=`) also shipped to support message matching.
...
**Deferred post-v1**: the original "Tasks" model (stackless state machines, `Handle<T>`, `await`, structured concurrency). With task borrows dropped (see `CONCURRENCY.md`), tasks and processes have identical ownership rules (move/clone at all spawn boundaries). The only difference is overhead (state machine vs. real stack). Tasks may be revisited if process overhead proves too high once the runtime matures.

**Open design**: `fn main` needs to be a process with a mailbox so it can `receive` replies from spawned children. Mailbox typing for main is an active design question -- union types (`Process<A | B>`) are the leading candidate. See `CONCURRENCY.md` for the full exploration.

See `archive/20260313-CONCURRENCY.md` and `CONCURRENCY.md` for full design details.
...
- **Borrow checker complexity**: Koja's model is simpler than Rust's (no lifetimes, no mutable borrows), but still requires flow analysis. Start with a conservative checker that rejects some valid programs rather than accepting invalid ones. Loosen over time.
...
- Mailbox typing for main is an open design question -- union types (`Process<A | B>`) are the leading candidate (see `CONCURRENCY.md`)
...
Implement natively in Koja (or Rust for the bootstrap) wherever possible. Use thin C FFI only for security-critical crypto and performance-critical parsing.
...
- ~~`koja run` for development (compile + execute)~~ -- implemented during Phase 1
- ~~`koja fmt` -- opinionated, zero-config code formatter~~ -- `koja format --write` / `--check` implemented during Phase 1
- ~~VS Code extension~~ -- syntax highlighting for `.koja` files implemented during Phase 1

### Package manager and project system

- `koja build` compiles a project based on `project.koja`
- `koja test` discovers and runs `@test` annotated functions
- Dependency resolution: fetch from hosted sources (git URLs initially, registry/mirror possible long-term)
- Lock file generation for reproducible builds
- **Done when**: `project.koja` from this repo resolves its three dependencies and builds the project
...
Rewrite the Koja compiler in Koja.

### Port the lexer and parser

- Rewrite the lexer and parser from Rust to Koja
...
- See "KojaIR and codegen backend protocol" in the design exploration section for full rationale.
...
- **Timing**: the IR split is Phase 6 (self-hosting) work. The current crate boundaries (codegen depends on ast + typecheck, clean downward dependencies) already support this separation. Keeping `koja-codegen` internals organized now avoids a painful refactor later.
...
| Tooling   | Formatter (`koja format --write`/`--check`)                                              | Done   |
| Tooling   | `koja run` (compile + execute)                                                           | Done   |
| Tooling   | VSCode extension (syntax highlighting)                                                   | Done   |
| Tooling   | LSP -- diagnostics, formatting, hover, go-to-definition                                  | Done   |
| Tooling   | Documentation generator (`koja doc`) -- HTML output, sidebar nav, brand theme            | Done   |
...
| Core      | Processes -- `spawn`/`receive`/`Process<M>`, `koja-runtime` scheduler, string equality   | Done   |
| Core      | Collections -- `List<T>`, `Map<K,V>`, `Set<T>`, `for` loops, map/set literals            | Done   |
| Core      | `List<T>` iterator functions -- `map`, `filter`, `any?`, `all?` (pure Koja)              | Done   |
...
| Stdlib     | Core types, I/O, time, `config.koja` compiles                                    |
...
| Tooling    | Interactive shell (`koja shell`) -- REPL with project loading                    |
| Self-host  | Lexer + parser in Koja                                                           |
| Self-host  | KojaIR + codegen backend protocol (`CodeEmitter`)                                |
| Self-host  | Full compiler in Koja                                                            |
| Self-host  | Retire Rust bootstrap                                                            |
| Validation | auth-manager-koja runs for real                                                  |
