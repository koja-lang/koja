...
- **A4 (lexer port)** is the sole remaining item. The validation milestone: Koja can write its own lexer, compiled by the Rust bootstrap, producing identical token output to the Rust lexer for all `.koja` test files.
...
- A3c test runner (`@test` functions, `koja test`)
...
- Package manager (`project.koja` dependencies, resolution, lock file)
...
| 6     | Self-hosting (parser port, KojaIR, full compiler, retire bootstrap) |
| 7     | Validation (auth-manager-koja, second project)                      |
