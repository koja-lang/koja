...
What the alpha pipeline (`koja-parser` → `koja-alpha-typecheck` →
`koja-alpha-ir` → `koja-alpha-ir-llvm` / `koja-alpha-ir-eval`) still
...
- **Qualified packages (`lib/<pkg>/src/`)**:
  - `Crypto` — parity. Wired into `QUALIFIED` and exercised by
    `alias Crypto.Sha256` in driver tests.
  - `Json` — language-feature parity. String interpolation (shipped 20260513) was the last gap; ready to wire into `QUALIFIED`
  ...
  - `Http` — language-feature parity. `String.clone()` (shipped
    ...
    names (shipped 20260513), and string interpolation (shipped 20260513) all landed; ready to wire into `QUALIFIED` once
  ...
  - `Net` — language-feature parity. Type unions (shipped 20260513) was the last gap; ready to wire into
    `QUALIFIED` once the source files clean-compile end-to-
...
- **End-to-end concurrency execution** — shipped 2026-05-13. See the
  ...
  spawns the user body as PID 1, runs `koja_rt_main_done()`, and
  ...
  goldens as runnable `.kojs` scripts (no `fn main`, no
  `koja.toml`) is the lighter-weight shape going forward,
  especially for concurrency snippets where the v1 project +
  manifest overhead was the bulk of the file.

- **Project mode in `koja alpha`** — shipped 2026-05-13. `koja alpha
{check,build,run}` now read `koja.toml`, walk the manifest's `src/`
  tree (including dependencies), parse + bundle through
  `bundle_many_with_autoimport`, run alpha typecheck, then lower via
  `koja_alpha_ir::lower_program` and link via the same `pipeline::link`
  v1 uses. Single-file `.koja` programs share the project pipeline
...
- `koja-alpha-typecheck/src/pipeline/resolve/{expr,statements,calls/mod,patterns/mod,literals/binary,closures,match_expr}.rs`
- `koja-alpha-typecheck/src/pipeline/seal/{expressions,patterns}.rs`
- `koja-alpha-ir/src/lower/{expr,ops,structs,enums,package,calls,closures}.rs`
...
`tests/lang/` against `koja alpha run --backend=llvm` now reports
...
- **Field-as-callable dispatch** (`self.work()` against a fn-typed
  field) — `process` is in `AUTOIMPORT`; `Task<R>.run` lowers.
- **Aliases** (`alias Pkg.Type as X`) — `alias_dep`,
  `package_collision`.
- **`Clone` protocol** for heap primitives — shipped 2026-05-12;
  pinned by `crates/koja-alpha-typecheck/tests/clone.rs`.
...
self`, `p.x += 1`) — shipped 2026-05-13. Resolver walks the
  ...
  Pinned by `crates/koja-alpha-typecheck/tests/field_assignment.rs`,
  `crates/koja-alpha-ir/tests/lower_field_assignment.rs`,
  `crates/koja-alpha-ir-eval/tests/field_assignment.rs`, and
  `crates/koja-alpha-ir-llvm/tests/field_assignment.rs`.
- **Dotted type names** (`Crypto.SHA256` and `HTTP.Headers` in
  ...
  `crates/koja-alpha-typecheck/tests/aliases.rs` and the
  `dotted_type_in_signature_lifts_to_qualified_global` test in
  `crates/koja-alpha-typecheck/tests/lift_function_types.rs`.
...
"\""`). IR lowering, eval, and LLVM all see only `String`-typed
  ...
  tests in `crates/koja-alpha-typecheck/tests/resolve_strings.rs`.
- **Type unions** (`A | B`, `type X = A | B`, typed-binding patterns
  ...
  `koja-ast/src/coercion.rs` for the 1:1 IR mapping). Match arms
...
symbol, tag, payload }` and dispatches `UnionWrap` / `TagGet` /
  ...
  `crates/koja-alpha-typecheck/tests/resolve_unions.rs` and
  `tests/resolve_type_aliases.rs`,
  `crates/koja-alpha-ir/tests/lower_unions.rs`,
  `crates/koja-alpha-ir-eval/tests/unions.rs`, and
  `crates/koja-alpha-ir-llvm/tests/unions.rs`.
- **Tail-call optimization** — shipped 2026-05-13. A new
  ...
  `koja-alpha-ir/src/tail_calls.rs` rewrite walks every regular
  ...
  tests in `koja-alpha-ir`,
  `crates/koja-alpha-ir-eval/tests/tail_calls.rs`, and
  `crates/koja-alpha-ir-llvm/tests/tail_calls.rs`.
- **FFI surface (`@extern "C"` + `CPtr<T>`)** — shipped 2026-05-13.
  See §7 for the full breakdown. Pinned by the `extern_*` snapshots
  in `crates/koja-alpha-ir-llvm/tests/extern.rs` and the `@extern
"C"` + `self` rejection in
  `crates/koja-alpha-typecheck/tests/extern_c.rs`.
...
- **End-to-end concurrency execution** — shipped 2026-05-13. See §8
  ...
  - `Pair<M, Option<ReplyTo<R>>>` envelope, protocol default-method
    ...
    `crates/koja-alpha-ir-llvm/tests/process.rs`.
...
loop"`. Closure boundaries save/restore both fields, so an inner
  ...
  `crates/koja-alpha-typecheck/tests/resolve_loops.rs`,
  `crates/koja-alpha-ir/tests/lower_loops.rs`,
  `crates/koja-alpha-ir-eval/tests/loops.rs`, and
  `crates/koja-alpha-ir-llvm/tests/loops.rs`.
...
- **Match-arm literal / partial / nested struct + enum patterns** —
  ...
  `crates/koja-alpha-ir-eval/tests/match_nested.rs`,
  `crates/koja-alpha-ir-llvm/tests/match_nested.rs`, and the
...
- **`String.print()` / `String.escape_debug()` runtime crash** —
  ...
  `Compiler::emit_print_call`, `__koja_alpha_print_{i64,bool,f32,
f64,binary,bits}`, `Interpreter::format_via_debug`'s driver
  ...
  output via `IO.puts` / `.print()`. The `__koja_user_main`
  spawn thunk caps with `ret void` and the `@main` trampoline
  caps with `ret i64 0`. Pinned by
  `crates/koja-alpha-ir/tests/lower_drops.rs`
  ...
  `crates/koja-alpha-ir-eval/tests/escape_debug.rs`,
  `crates/koja-alpha-ir-llvm/tests/escape_debug.rs`, and the new
  `tests/lang/basics/string_match_escape.koja` golden.
...
Unblocked `structs.koja` and removed every alpha gap from the
...
`koja-alpha-ir/src/tail_calls.rs` rewrite that walks every
...
(`tail_call.koja` / `tail_call_unit.koja`) in constant host
...
`declare`s with the C-ABI `i32` / `i64` / `ptr` mapping (no Koja
...
`crates/koja-alpha-ir-llvm/tests/extern.rs` (alongside the existing
`extern_c_with_link_lib_declares_external_function` pin), plus the
`@extern "C"` + `self` rejection in
`crates/koja-alpha-typecheck/tests/extern_c.rs`. The
`tests/lang/ffi/` golden compiles and links cleanly through
`koja alpha build --backend=llvm` once `libffi_helper.a` is in
`LIBRARY_PATH`.
...
1. **Spawn-driven `main` trampoline**
   (`koja-alpha-ir-llvm/src/main_wrapper.rs`). The user body now
   lives in a `void __koja_user_main(i8*)` thunk, and the host
   `i64 main()` is a one-call trampoline:
   `koja_rt_spawn(__koja_user_main, null, 0)` registers the body as
   PID 1, `koja_rt_main_done()` boots the I/O reactor + scheduler
   and runs until PID 1 dies, then `ret i64 0`. Required for
   `koja_rt_self()`, `Ref.call`, `Ref.cast`, and every other
   primitive that reads `CURRENT_PID` — pre-refactor the body ran
   inline in `main()` with `CURRENT_PID == -1`, which crashed any
   concurrency code that reached `koja_rt_self()` in the runtime's
   process table. Pinned indirectly by the spawn-from-`main`
   integration tests in `crates/koja-alpha-ir-llvm/tests/process.rs`.
2. **`Ref.call` + `Pair<M, Option<ReplyTo<R>>>` envelope**
   (`koja-alpha-ir-llvm/src/intrinsics/process.rs`). `Ref.cast`
   and `Ref.send_after` now build the same `Pair` envelope the
   runtime's `koja_rt_receive_*` paths and the `Receive`
   ...
   with `Option::Some(ReplyTo { id: koja_rt_self() })`, sends via
   `koja_rt_send`, then `koja_rt_receive_timeout(timeout_ms)` to
   ...
   snapshots in `crates/koja-alpha-ir-llvm/tests/process.rs`. The
   `IRSymbol → IRStructDecl/IREnumDecl` lookup table was extended
   with a per-enum `IRVariantPayload` registry
   (`koja-alpha-ir-llvm/src/layout.rs` + `layout/enums.rs`) so
...
3. **Protocol default-method type-param substitution**
   (`koja-alpha-typecheck/src/pipeline/lift_signatures/impls.rs`).
...
`lib/global/src/alpha_debug_containers.koja` is parametric: its
...
[`koja_alpha_typecheck::pipeline::synthesize::derive_debug::is_opaque_type`]
...
`crates/koja-alpha-ir/tests/opaque_debug_receivers.rs`.

### Future direction: prefer `.kojs` for new goldens

Alpha runs `.kojs` scripts directly (no `fn main`, no `koja.toml`),
v1 doesn't. Going forward, new goldens for `tests/lang/` should
prefer the `.kojs` shape: `tests/lang/<feature>.kojs` +
`tests/lang/<feature>.stdout`, collapsing the per-fixture overhead
from a project directory to two files. Existing `.koja` goldens
...
toolchain (`koja-typecheck`, `koja-codegen`) can be removed.
