...
Design notes for the Koja self-hosted compiler. The Rust bootstrap compiler
...
With accumulators split out, TypeContext is immutable. Immutable data in Koja
...
| Rust workaround                                           | Lines | Status in Koja                                    |
| --------------------------------------------------------- | ----- | ------------------------------------------------- |
...
Koja doesn't need this. `send(registry, ModuleReady{name, move ctx})` transfers
...
├── DocumentActor (file_a.koja)  -- owns DocumentState, handles edits
├── DocumentActor (file_b.koja)  -- notified when file_a's exports change
├── DocumentActor (file_c.koja)  -- independent, unaffected by A/B changes
...
Beyond the architectural wins, Koja syntax eliminates per-line ceremony that
compounds across the codebase:

**No `Box<T>` for recursive types.** The Rust AST has `Box<Expr>` in 14+ places.
Every match arm dereferences through boxes. Every constructor boxes. Koja handles
heap placement automatically.

**No lifetime annotations.** `Compiler<'ctx>` appears in 80+ function signatures
in the codegen. In Koja, the LLVM context outlives the compiler by construction.

**No `iter()` / `into_iter()` / `iter_mut()` selection.** Koja's `for` loop
borrows by default. `move` to consume.

**No `.as_str()` / `.as_ref()` / deref coercion friction.** Koja has one string
...
Koja:    fn infer_expr(expr: Expr, ctx: TypeContext, diags: Ref<DiagMsg, List<Diagnostic>>) -> Type
```
...
Koja has no mutable borrows. For the variable environment (per-function
...
outlives its context. Koja can't express this. The self-hosted codegen must rely
