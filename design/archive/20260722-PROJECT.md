...
> Archived on 2026-07-22. This snapshot describes a proposed project layout
> that diverged from the shipped loader, dependency manager, test runner, and
> build outputs. See the current [project model](../PROJECT.md).
...
- The cookbook-not-packages distribution model from
  `archive/20260610-PACKAGE.md` (since archived). A conventional package
  manager with git dependencies and a committed `koja.lock` has since
  shipped. `koja.toml` dependencies share code between projects.
