...
`koja` binary, runs `koja-driver`'s `lang_suite` plus the stdlib
package suite with `KOJA_STUB_INVENTORY_FILE=$(pwd)/stub-{driver,stdlib}.log`
...
[`Lowerer::stage_union_widen`](../crates/koja-ir/src/lower/coercion.rs).
...
[`Lowerer::stage_arg_coercions`](../crates/koja-ir/src/lower/coercion.rs)
...
- **Phase 1 -- function attribution.**
  [`compile_function_body`](../crates/koja-codegen/src/generics.rs)
  ...
  [`populate_ir_blocks`](../crates/koja-codegen/src/compiler.rs)
...
- **Phase 2 -- per-branch bail logging.**
  [`koja-ir/src/lower/diag.rs`](../crates/koja-ir/src/lower/diag.rs)
...
- **Phase 3 -- typed coercion lowering for `MethodCall`.** The
  ...
  receiver and per-arg [`Coercion::UnionWiden`](../crates/koja-typecheck/src/context.rs)
  are now staged through
  [`Lowerer::stage_union_widen`](../crates/koja-ir/src/lower/coercion.rs)
  (the existing `maybe_emit_union_wrap` for assignment / return
  delegates to the same helper).
  [`emit_method_call_instruction`](../crates/koja-ir/src/lower/methods.rs)
...
- **Phase 3b -- codegen `apply_coercion` retirement: deferred.**
  ...
  [`compile_assignment`](../crates/koja-codegen/src/stmt.rs), and
...
- **Phase 1 -- `lower_static_call_or_stub` instrumentation.**
  ...
  [`lower_static_call_or_stub`](../crates/koja-ir/src/lower/calls.rs)
...
- **Phase 2 -- typed coercion lift across the rest of the call
  ...
  [`Coercion::UnionWiden`](../crates/koja-typecheck/src/context.rs)
  ...
  [`lower_call_or_stub`](../crates/koja-ir/src/lower/calls.rs)
...
([crates/koja-ir/src/lower/methods.rs](../crates/koja-ir/src/lower/methods.rs)):
...
7. `resolved.pending_mono.is_some()` -- monomorphization driver lives in koja-codegen
...
[`IRInstruction::UnionWrap`](../crates/koja-ir/src/values.rs)
staging.)
...
| ExprKind  | Count | Notes                                                                                                                                                                                                                                                                                                              |
| --------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Literal` | 34    | `resolve_const_inline` rejects some literal shapes (likely string-with-interpolation, or numeric literals whose `Type` slot needs the surrounding context). Inspect [crates/koja-ir/src/lower/constants.rs](../crates/koja-ir/src/lower/constants.rs) to enumerate what `resolve_const_inline` returns `None` for. |
...
1. **Closure-pass / symbol-registration drain
   ...
   [`IRProgram`](../crates/koja-ir/src/program.rs) before
...
- Retiring the codegen-side `IRInstruction::Stub` consumer at
  [crates/koja-codegen/src/control/instructions.rs](../crates/koja-codegen/src/control/instructions.rs)
  line 369.
